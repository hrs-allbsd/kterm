 ...
 *	$Id: input.c,v 6.4 1996/07/12 05:01:34 kagotani Rel $
 */
...
#define STRBUFSIZE 256
	...
	char _strbuf[STRBUFSIZE+1];
	char *strbuf = &_strbuf[1];
#else /* !KTERM */
#define STRBUFSIZE 100
	...
	if (!screen->xic) {
	    nbytes = XmuLookupKana (event, strbuf, STRBUFSIZE,
				    &keysym, NULL);
		if (keysym>>8 == 4 && keysym & 0x80 && nbytes == 1) {
		    /* fake ja_JP.eucJP & @im=none */
		    strbuf[-1] = SS2;
		    nbytes ++;
		    string = &strbuf[-1];
		} else {
		...
		if (nbytes == 1) {
		  ...
		  unparseputc(*string, pty);
			...
			if (term->flags & EUC_KANJI) {
				...
				string[nbytes] = '\0';
				n = (*func)(string, NULL);
				obuf = (n > STRBUFSIZE) ? (Char *)XtMalloc(n + 1) : outbuf;
				(void)(*func)(string, obuf);
				optr = obuf;
				while (*optr)
