 ...
 *	$Id: scrollbar.c,v 6.3 1996/07/12 05:01:34 kagotani Rel $
 */
...
# ifdef STATUSLINE
	min_height += screen->statusheight;
# endif /* STATUSLINE */
#ifndef nothack
	/*
    ...
    int init, doalloc;
{
	...
	if (doalloc && screen->allbuf) {
	    ...
	    memmove( (char *)screen->buf, (char *)screen->allbuf, 
		   (screen->max_row + 2) * sizeof (Bchr *));
	    for(i = screen->savelines - 1 ; i >= 0 ; i--) {
	      if((screen->allbuf[i] =
		  (Bchr *)calloc((unsigned) screen->max_col + 1, sizeof(Bchr))) == NULL)
    ...
    Cardinal *nparams;
{
    ...
    if (SendMousePosition(gw, event) != 0)
	return;
